...
public class Toaster extends Handler {
    ...
    private Toaster() {
        superToastPriorityQueue = new PriorityQueue<>(10, new Toaster.SuperToastComparator());
    }
    ...
    private void showNextSuperToast() {
        ...
        if (!superToast.isShowing()) {
            final Message message = obtainMessage(Toaster.Messages.DISPLAY_SUPERTOAST);
    ...
    public void handleMessage(Message message) {
        ...
        switch (message.what) {
            case Toaster.Messages.SHOW_NEXT:
                showNextSuperToast();
                break;
            case Toaster.Messages.DISPLAY_SUPERTOAST:
                displaySuperToast(superToast);
                break;
            case Toaster.Messages.REMOVE_SUPERTOAST:
    ...
    private void displaySuperToast(SuperToast superToast) {
        ...
        if (superToast instanceof SuperActivityToast) {
            ...
            if (!((SuperActivityToast) superToast).isIndeterminate()) {
                ...
                sendDelayedMessage(superToast, Toaster.Messages.REMOVE_SUPERTOAST,
    ...
    void removeSuperToast(final SuperToast superToast) {
        ...
        if (superToast instanceof SuperActivityToast) {
            ...
            this.sendDelayedMessage(superToast, Toaster.Messages.SHOW_NEXT, AnimationUtils.HIDE_DURATION);
        }
    ...
    void cancelAllSuperToasts() {
        removeMessages(Toaster.Messages.SHOW_NEXT);
        removeMessages(Toaster.Messages.DISPLAY_SUPERTOAST);
        removeMessages(Toaster.Messages.REMOVE_SUPERTOAST);
