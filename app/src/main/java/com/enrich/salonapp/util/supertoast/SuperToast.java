...
import com.enrich.salonapp.util.supertoast.utils.AccessibilityUtils;
import com.enrich.salonapp.util.supertoast.utils.AnimationUtils;
...
public class SuperToast {
     ...
     *
     * @see #setOnDismissListener(String, android.os.Parcelable, SuperToast.OnDismissListener)
     */
    ...
    public SuperToast(@NonNull Context context) {
        ...
        this.mTextView = this.mView.findViewById(R.id.message);
        this.mImageView = this.mView.findViewById(R.id.image);
    }
    ...
    protected void onPrepareShow() {
        ...
        if (sdkVersion >= Build.VERSION_CODES.JELLY_BEAN) {
            if (this.mStyle.messageImageURL == null)
                this.mTextView.setBackground(BackgroundUtils.getBackground(this.mStyle, this.mStyle.color));
            else
                this.mTextView.setBackgroundColor(Color.TRANSPARENT);
            // Give Lollipop devices a nice shadow (does not work with transparent backgrounds)
            if (sdkVersion >= Build.VERSION_CODES.LOLLIPOP) mView.setElevation(0f);
        } else {
            if (mImageView == null)
                this.mTextView.setBackground(BackgroundUtils.getBackground(this.mStyle, this.mStyle.color));
            else
...
//            this.mView.setBackgroundDrawable(BackgroundUtils.getBackground(this.mStyle, this.mStyle.color));
        ...
        if (this.mStyle.frame == Style.FRAME_LOLLIPOP) {
            ...
            if ((this.mContext.getResources().getConfiguration().screenLayout & Configuration.SCREENLAYOUT_SIZE_MASK)
                ...
                gradientDrawable.setCornerRadius(BackgroundUtils.convertToDIP(0));
    ...
    public SuperToast setImageURL(String url) {
        this.mStyle.messageImageURL = url;
        return this;
    }
    ...
    protected SuperToast setOnDismissListener(String tag, Parcelable token,
                                              @NonNull SuperToast.OnDismissListener onDismissListener) {
     ...
     * @return The current SuperToast instance
     */
    public SuperToast setOnDismissListener(@NonNull SuperToast.OnDismissListener onDismissListener) {
        this.mOnDismissListener = onDismissListener;
    ...
    /**
     * Returns the {@link SuperToast.OnDismissListener}
     * of the SuperToast.
     *
    ...
    /**
     * Sets the {@link Style} of the SuperToast.
     *
     * @param style The desired Style
     * @return The current SuperToast instance
     */
    ...
    /**
     * Returns the {@link Style} of the SuperToast.
     *
     * @return The Style
     */
     ...
     * {@link Toaster#displaySuperToast(SuperToast)}
     */
